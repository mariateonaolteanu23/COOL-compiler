...
public class ASTConstructionVisitor extends CoolParserBaseVisitor<ASTNode> {
    ...
    public ASTNode visitProgram(CoolParser.ProgramContext ctx) {
        return new Program(ctx.children.stream().map(this::visit).filter(Objects::nonNull).collect(Collectors.toList()), ctx.start);
    }
    ...
    public ASTNode visitAttribute(CoolParser.AttributeContext ctx) {
        if (ctx.init == null)
            return new Attribute(new Id(ctx.name), new Type(ctx.type), ctx.start);
        return new Attribute(new Id(ctx.name), new Type(ctx.type), (Expression)visit(ctx.init), ctx.start);
    }

    @Override
    public ASTNode visitNew(CoolParser.NewContext ctx) {
        return new New(new Type(ctx.type), ctx.start);
    }
    ...
    public ASTNode visitBitComplement(CoolParser.BitComplementContext ctx) {
        return new BitComplement((Expression) visit(ctx.e), ctx.TILDE().getSymbol());
    }

    @Override
    public ASTNode visitPlusMinus(CoolParser.PlusMinusContext ctx) {
        if (ctx.op.getText().equals("+"))
            return new Plus((Expression)visit(ctx.left), (Expression)visit(ctx.right), ctx.op);

        return new Minus((Expression)visit(ctx.left), (Expression)visit(ctx.right), ctx.op);
    }
    ...
    public ASTNode visitWhile(CoolParser.WhileContext ctx) {
        return new While((Expression) visit(ctx.cond), (Expression) visit(ctx.body), ctx.WHILE().getSymbol());
    }

    @Override
    public ASTNode visitImplicitDispatch(CoolParser.ImplicitDispatchContext ctx) {
        return new ImplicitDispatch(null, new Id(ctx.name),
    ...
    public ASTNode visitNot(CoolParser.NotContext ctx) {
        return new Not((Expression) visit(ctx.e), ctx.NOT().getSymbol());
    }
    ...
    public ASTNode visitMultDiv(CoolParser.MultDivContext ctx) {
        if (ctx.op.getText().equals("*"))
            return new Mult((Expression)visit(ctx.left), (Expression)visit(ctx.right), ctx.op);
        return new Div((Expression)visit(ctx.left), (Expression)visit(ctx.right), ctx.op);
    }
    ...
    public ASTNode visitIf(CoolParser.IfContext ctx) {
        return new If((Expression) visit(ctx.cond), (Expression) visit(ctx.thenBranch), (Expression) visit(ctx.elseBranch), ctx.IF().getSymbol());
    }
    ...
    public ASTNode visitAssign(CoolParser.AssignContext ctx) {
        return new Assign(new Id(ctx.name), (Expression) visit(ctx.e), ctx.ASSIGN().getSymbol());
    }

    @Override
    public ASTNode visitCaseBranchDef(CoolParser.CaseBranchDefContext ctx) {
        return new CaseBranch(new Id(ctx.name), new Type(ctx.type), (Expression) visit(ctx.e), ctx.start);
    }

    @Override
    public ASTNode visitLocalDef(CoolParser.LocalDefContext ctx) {
        if (ctx.init == null)
            return new Local(new Id(ctx.name), new Type(ctx.type), ctx.start);
        return new Local(new Id(ctx.name), new Type(ctx.type), (Expression) visit(ctx.init), ctx.start);
    }
